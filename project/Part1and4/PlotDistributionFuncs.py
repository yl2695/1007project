...
def StateSearch(df, state_name):
    '''
    df is a DataFrame
    state_name is a state abbreviation from {'ON', 'ELN', 'EDH', 'MLN', 'WI', 'NY', 'KHL', 'AZ', 'CA', 'NV'}
    search for restaurant information in a state
...
def SaveToDict(df):
    ...
    It returns the possible star values and the number of restaurants with the corresponding star value
    For example:
...
def PlotStarDistribution():
    '''
    This function returns 6 histograms to show the distributions of stars in six states: ON, EDH, MLN, WI, AZ, NV
    ...
    for state in ['ON', 'EDH', 'MLN', 'WI', 'AZ', 'NV']:
        ...
        # counts, bins=np.histogram(df, bins=11, normed=False)

        df.hist(bins=200, ax=axes[i], color='k', alpha=.5)
        axes[i].set_title(state)

        # axs[i].set_xticks(range(0, 5.1, .5))
        # axs[i].set_xticklabels(range(0, 5.1, .5), rotation=60, fontsize=7)
        # axs[i].set_yticks(range(0, 5.1, .5))
        # axes[i].set_yticklabels(rotation=30, fontsize=6)
...
def PlotPieChartForOneState(state):
    '''
    This function plot a pie chart for a certain state

    Note: Though this function can plot a pie chart for any state in {'ON', 'ELN', 'EDH', 'MLN', 'WI', 'NY', 'KHL', 'AZ', 'CA', 'NV'}
    ...
    pie_labels = [[x, y] for x, y in zip(star_count.keys(), percent)]
